...
async def test_zigbee2_router_button(
    ...
    state = hass.states.get("button.mock_title_reconnect_zigbee_router_2")
    assert state is not None
    assert state.state == STATE_UNKNOWN

    entry = entity_registry.async_get("button.mock_title_reconnect_zigbee_router_2")
    assert entry is not None
    assert entry.unique_id == "aa:bb:cc:dd:ee:ff-reconnect_zigbee_router2"
